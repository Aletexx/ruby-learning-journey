...
class Game
  ...
  def display_game_board(player)
    ...
    loop do  #Loop forever
      ...
      #Q2
      if move == "H" then puts display_statistics end
      ...
      if move.length == 2 then  #Must be at 2 character long
        ...
        not_valid =  display_error("a", "0", "0")
        puts not_valid
        elsif move == player.move then
          not_val =  display_error("0", "b", "0")
          puts not_val
        end
      end
      end
  ...
  def display_error(inv_char, rep_move, more_than_three_char)
    if inv_char == "a" then puts "Invalid character" end
    if rep_move == "b" then puts "move already used" end
    if more_than_three_char == "c" then puts "Sorry more than 3 characters" end
  end
  ...
  def display_statistics
    puts "Player X won: #{$xWins} "
    puts "Player 0 won: #{$oWins} "
  end
