...
class Game
  ...
  def get_player_move
    ...
    loop do
      ...
      print "[Rock] or [R] , [Paper] or [P] , [Scissors] or [S] : "
      ...
      break if @choice =~ /Rock|Paper|Scissors|R|P|S/i
  ...
  def analyze_results(player, computer)
    ...
    if player == "ROCK" or player == "R" then
      ...
      return "Player wins! Rock smashes Scissors"
       ...
       if computer == "PAPER"
      $lost += 1
       return "Computer wins! Paper against Rock"
      end
    end

    #Analyze the results of the game when the player selects PAPER
    if player == "PAPER" or player == "P" then
      if computer == "ROCK"
        $wins = $wins + 1
        return "Player wins! Paper wins over rock!"
      end
      if computer == "PAPER"
        $tie += 1
    ...
    if player == "SCISSORS" or player == "S" then
       if computer == "PAPER"
         $wins = $wins + 1
